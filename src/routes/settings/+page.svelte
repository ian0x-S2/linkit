<script lang="ts">
	...
	async function confirmDeleteWorkspace() {
		...
		try {
			...
			const wasActive = store.workspaces.activeId === id;
			await store.workspaces.remove(id as WorkspaceId);
			if (wasActive && store.workspaces.count > 0) {
				await store.setActiveWorkspace(store.workspaces.workspaces[0].id);
			}
		...
		<div class="mx-auto max-w-2xl space-y-8 p-6">
			...
			<section class="space-y-4">
				<div class="flex items-center justify-between px-1">
					<div class="space-y-1">
						<h3 class="text-[10px] font-bold tracking-widest text-muted-foreground/60 uppercase">
							Management
						</h3>
				...
				<div class="grid gap-3">
					{#each store.workspaces.workspaces as ws (ws.id)}
						<div
							class="group flex items-center justify-between rounded-md border bg-muted/20 p-3 transition-colors hover:bg-muted/30"
						>
							<div class="flex items-center gap-3">
								<div
									class="flex h-8 w-8 items-center justify-center rounded-md border bg-background shadow-sm"
								>
									<Layers class="h-4 w-4 text-muted-foreground" />
								</div>
								<div class="space-y-0.5">
									<div class="flex items-center gap-2">
										<p class="text-[13px] leading-none font-medium">{ws.name}</p>
										{#if ws.id === store.workspaces.activeId}
											<Badge variant="secondary" class="h-4 px-1.5 text-[9px] font-bold uppercase"
												>Active</Badge
											>
										{/if}
									</div>
							...
							<div class="flex items-center gap-2">
								{#if store.workspaces.count > 1}
									<Button
										variant="ghost"
										size="icon"
										class="h-8 w-8 rounded-md text-muted-foreground opacity-0 transition-opacity group-hover:opacity-100 hover:bg-destructive/10 hover:text-destructive"
			...
			<section class="space-y-3 px-1">
				...
				<div class="flex items-center justify-between py-1">
					...
					<Badge
						variant="outline"
						class="h-4.5 rounded-lg border-green-500/50 px-1.5 text-[9px] font-bold tracking-tighter text-green-600 uppercase dark:text-green-400"
						>Connected</Badge
					>
				</div>
			</section>
			...
			<div class="flex justify-end gap-2 border-t pt-6">
				<Button variant="ghost" href="/" class="h-8 rounded-md px-4 text-[12.5px]"
	...
	<Dialog.Content class="max-w-100">
		<Dialog.Header>
			...
			<Dialog.Description class="text-[13px]">
				...
				>? This action cannot be undone and all links within this workspace will be permanently
				removed.
			</Dialog.Description>
		</Dialog.Header>
